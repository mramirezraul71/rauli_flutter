...
import '../../main.dart' show AppStateScope, AppState; // usamos tu state ya existente
          ...
          children: [
            ...
            if (items.isEmpty)
              const _EmptyCard(
                title: "Sin items",
                subtitle:
                    "Crea items desde cero o importa un CSV (SKU, Nombre, Unidad, Stock, Mínimo).",
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: isLow ? Colors.red.withOpacity(0.12) : Colors.black.withOpacity(0.06),
            borderRadius: BorderRadius.circular(14),
          ),
          child: Icon(isLow ? Icons.warning_amber_rounded : Icons.inventory_2_rounded),
        ),
        const SizedBox(width: 12),
        Expanded(
...
class _ImportCsvDialogState extends State<_ImportCsvDialog> {
        ...
        FilledButton(
          onPressed: () {
            ...
            for (final row in parsed) {
              ...
              final itemId = "i_${DateTime.now().millisecondsSinceEpoch}_$created";
              ...
              state.addInventoryItem(it);
              created++;

              if (stock > 0) {
                state.addStockMove(StockMove(
                  id: "m_${DateTime.now().millisecondsSinceEpoch}_$created",
                  ts: DateTime.now(),
                  itemId: itemId,
            ...
            setState(() => status = "Importado: $created items, $moved stocks.");
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("Importado: $created items • stock aplicado: $moved")),
            );
          },
  ...
  /// Parser CSV simple (coma o punto y coma)
  List<Map<String, String>> _parseCsv(String input) {
    final lines = const LineSplitter().convert(input).where((l) => l.trim().isNotEmpty).toList();
    if (lines.length < 2) return [];

    final delimiter = lines.first.contains(";") && !lines.first.contains(",") ? ";" : ",";
    final header = lines.first.split(delimiter).map((s) => s.trim().toLowerCase()).toList();
